...
- **D4**：核心操作 3 步内完成，组件复用优先于定制开发
- **D5**：只定义交互行为，不涉及外观（fe-system）和技术实现（detail）
- **D7**：每个流程必须有异常处理路径，操作必须有反馈
...
**不应包含**：视觉设计（fe-system）· 技术实现（detail 阶段）· 组件外观（fe-system）
## 模板
