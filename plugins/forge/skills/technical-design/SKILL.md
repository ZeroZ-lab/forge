...
**不变原则**：团队经验 > 技术先进性 · 生态完善 > 性能极致 · 每个依赖都要有明确的使用方（不列没人用的依赖）

**依赖纪律**：技术选型表每个依赖必须有——版本、使用方（哪个服务/模块/决策用它）、是否由其他依赖内置（如 R3F 内置 Zustand，避免重复安装且需版本一致）。未被任何模块使用的依赖不列入。

**记录**：技术栈清单（每项含版本 + 使用方 + 内置标注）+ 选择理由 + 被拒方案
...
1. **架构模式表** — 选择 × 理由 × 被拒方案
2. **技术选型表** — 层 × 选择 × 版本 × 使用方 × 内置于 × 理由 × 被拒方案
...
- 没有被拒方案 → 强制补充（"为什么不用 X？"）
- 依赖没有版本 / 没有使用方 / 没标注是否被内置 → 强制补充（"这个依赖哪个模块用？版本多少？是不是已被其他依赖内置？"）
