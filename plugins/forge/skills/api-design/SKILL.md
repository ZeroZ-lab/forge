...
**下游**：goal.md（API1-API7 决策 + 共享数据模型）+ modules/*.md 交给 db-design（数据模型）、frontend-design（消费接口）、plan 阶段（任务分解）
**和 db-design 的切法**：
...
## 决策点
> API 领域的决策编号使用 API# 前缀（API Decision）。项目级共享决策使用 PD# 前缀，记录在 project.md。Feature 级共享决策使用 FD# 前缀，记录在 feature/goal.md。数据库领域使用 DB# 前缀，记录在 goal.md。
### API1: 资源建模
...
**出口**：goal.md（API1-API7 决策 + 共享数据模型）+ modules/*.md（接口合约完整）
...
1. **追加 feature changelog.md**：
   ...
   ### v{版本} — {日期} — API 详设
   - **触发**：{用户说的一句话}
   - **产出**：goal.md（API1-API7 决策）+ {N} 个 modules
   ```
2. **追加 docs/timeline.md**：
   ```markdown
   ### {日期} — {feature} API 详设
   - 新增：goal.md（API 决策）+ modules/
   ```
3. **检查膨胀**：超 100 行时归档。
## 完成提示
```
✅ API 详设完成！goal.md + modules/ 已生成。
