...
- **Invocation mode**: direct action, standalone Skill invocation, or child of the Chain Owner.
...
2. For one user objective, the root agent is the Chain Owner. A child Skill does not persist history or own global completion. It returns changed files, decisions, risks, verification evidence, unresolved items, and an advisory next action; the Chain Owner is the only writer for the consolidated record.
...
4. A direct or Chain Owner-coordinated run writes one consolidated Change Unit when it completes with retained mutation. If it is blocked after mutation and retains changes, the Chain Owner still writes one partial record; if all changes are rolled back, the no-retained-mutation rule applies.
...
11. The Change Unit is an evidence sink after execution, not a workflow controller. Its existence does not require a fixed Skill path, and Skill activation is not completion evidence.
