...
> 这里只放**跨组件共享**的类型。单个组件的 Props 接口定义在该组件自己的 module spec（`modules/<Name>.md` 的「公共接口」节）里，本文件引用类型名，不重写签名——避免同一 Props 两处定义不一致（单一权威，见 shared/concepts/reference-not-repeat.md）。跨领域共享的数据模型在 goal.md。
...
> 完整技术栈见 project.md，以下为前端补充依赖。每个依赖标注使用方，不列没人用的依赖。
