...
- D4/D5：只改当前任务必要文件；不加未要求抽象/依赖；无关问题只记录。
- D7/D9：完成前必须有可校验证据：命令 + 真实输出，不是“测试通过”。无法运行写 `⚠️ 未验证` + 阻塞原因。
- D8：同类问题 ≥2 次，回 detail 复查目标。

## 读→生→验→修

**读**：先读 project/goal/modules/任务/代码/历史风险；识别 PD#/FD#/DB#/AC；跳读直接写 = 目标漂移。

**生**：无任务序列则从 goal 推最小顺序并声明。goal 推 routes/schemas/db/tests；modules 推 services/unit tests；project 推目录/配置；DESIGN 推前端。关键逻辑标 FD#/PD#/DB#/AC#。Bugfix 先建 red-capable 复现；无反馈循环不改实现。

**验**：先运行验证，再目标对照。查编译/语法、启动或核心入口、核心路径响应、相关测试、goal/modules 对照（端点、字段、状态码、权限、幂等）。运行验证失败或无命令回执，不得完成。

**修**：失败归因代码、文档歧义、文档未同步、范围蔓延；修后回到验证；3 轮不收敛交用户。摘要：`自动修正 N · 中止 M · 待确认 K · 归因：skill / 文档 / 代码 / 范围`。

## 红旗/出口

红旗：未读目标改代码；无运行验证却完成；代码与 goal/modules 不一致；测试不覆盖 AC；bugfix 无 red seam；关键逻辑无编号；新增 goal 外功能/依赖；共享约束漏注入。
