...
把 goal/module 中的完成标准转成可执行 BDD 场景矩阵。test-cases 不发明需求，不维护第二份验收事实。
...
2. 每个场景用 `AC# / Given / When / Then / 层级 / 是否自动化 / 风险` 表达，注明来源 AC、前置数据、动作、可观察预期和建议测试层级。
3. 识别最小回归用例与高风险数据组合。
4. 已自动化的场景直接落到测试代码；未自动化场景留在当前执行上下文。

## 产出

默认在对话或 issue 中返回 BDD 场景矩阵，并在用户要求实现时写入 `tests/`。遵循 `${CLAUDE_SKILL_DIR}/../shared/concepts/artifact-policy.md`：不创建 `testing/test-cases.md` 或 `.feature` 用例库。
