...
以下为 D4–D10 在执行层面的展开。完整定义见「操作纪律（D1–D10）」。
...
### 8 阶段（含 2 子阶段，共 10 个阶段标记） × 19 个领域 Skill + 4 个编排 Skill + 1 个思考增强 Skill
...
方向简报  →  项目章程 → PRD  →  算法菜单  →    交互规格 + 设计系统 + 技术方案    →    goal.md + modules/
                                                                              ...
                                                                              ↓
⑦ 交付          ⑥.5 审查        ⑥ 测试                     ⑤ 构建         ④ 任务
发布清单  ←   审查报告（→ learn 归档）   ←   测试策略 + 测试用例   ←   src/ + tests/  ←   plan.md（任务分解）
```
...
Forge 的验证理念：文档定义目标，AI 自主实现，再验证结果是否达标。验证发生在两个环节：

| 环节 | 做法 |
|------|------|
| **实现** | codegen 读取目标 → 生成代码 → 验证目标是否达成。同一完成标准项在单次 session 中连续验证不通过 ≥ 2 次 → 停下来建议复查目标定义 |
| **审查** | review 检查实现是否满足目标 → 差距分析。目标冲突或 P0/P1 阻塞 → 交人类决策 |
...
启用 status.md 后，编排器完成时，在「历史维护」步骤中同步更新 status.md：
