...
| PD15 | 四臂计划与模型调度 | manifest v4 固定 `no-forge`、`kernel-only`、`adaptive-full`、`legacy-chain` 四种唯一 launch policy；adaptive 绑定当前发布 Skill tree 且允许零调用，legacy 绑定 Forge 0.52.0 tree 和升级前默认链；B05 把同组输入/预算/limits/verifier 的 common context 与唯一 arm context 注入实际 command 并绑定 B04 fingerprint，只接受显式 model，正式 actual 来自 B04 保留的子进程 transport receipt 并精确匹配 preflight selection；runtime/host receipt 逐臂落盘，host adapter 回绑同一 policy 和 containment，四臂只有在公平性与 cleanup 通过后才写 `group.json` completion seal | 将能力暴露限定为唯一实验变量，既能测 Kernel/自适应增益又不束缚更强模型；旧链保留可复现实验对照，运行宿主的安全责任不被 Node process group 伪装，部分报告不会冒充完整比较 | 继续使用歧义 `forge` arm；由 report/provider callback 自报 actual/exposure；按 arm 改预算或 verifier；默认/回退模型；未 seal 的部分组；把 transport receipt 当远端真实性证明；把 `ulimit`、workspace clone 或 deprecated `sandbox-exec` 宣称为完整隔离 |
