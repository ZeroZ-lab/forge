...
| Internal coordinator | 在一个能力内部协调多个 child，并向 Chain Owner 返回一份汇总证据；只有它本身也是根 Chain Owner 时才写 CU |
| Kernel | 只管理目标、权限、范围、权威事实、证据、任务状态和完成条件的外部控制边界 |
| Direct action | 模型不调用 Skill，直接执行并验证当前目标所需动作的一等路径 |
| Chain Owner | 同一用户目标的根代理；唯一维护全局状态、review 独立性、最终完成声明与汇总 Change Unit |
| Adaptive capability | 模型按边际价值选择的可选 Skill；可以任意组合或零调用，不能成为完成条件 |
...
| # | 决策 | 选择 | 理由 | 拒绝 |
|---|------|------|------|------|
| PD1 | 默认运行方式 | Kernel-first adaptive：目标清晰时直接行动；按边际价值调用任意、多个或零个 Skill；L2/L3 或 P0/P1 独立复核 | 不给更强模型设置固定能力上限，同时保留目标、权限、范围、证据和完成硬门 | 固定 `detail → codegen → review` 生产主链；默认完整生命周期；无约束自主 |
...
| PD9 | 阻塞后的变更证据 | direct/standalone 有保留变更时由当前 Chain Owner 写一个部分完成 CU；child 只交证据包；完全回滚则不写 | 长程或失败任务不能丢失已发生变更、未验证项和回滚证据，同时避免 child 重复写入 | child 自写 CU；阻塞后一律不记录；无变更也写 CU |
...
| PD17 | 外部 verifier host | B07 把 test/build/typecheck、host-private hidden assertion 与 captured-diff 检查归一成版本化 verifier runtime；verifier-set digest 绑定 host definition 和全部 adapter definition。只有 B05 注入且与受控 verifier set 相符的 runtime 能运行；host contract 必须声明 timeout/output、CPU/内存/磁盘、网络、secret、workspace/evidence、process-tree、non-blocking bridge 与 cancellation 保证，Forge 用 deadline + cancel acknowledgement fail closed，并向 diff verifier 提供 retained diff 与 base snapshot handle。B04 在 cleanup 前执行和复查输入，首次 report 写 independent result/observation 与 B06 claim Envelope；B05 seal v3 绑定完整有序 verifier manifest、从 observation 重推 outcome，v1/v2 保持原语义。结果不判断目标 outcome | 让 verifier failure 成为不能被模型后续文本覆盖的运行事实，同时把 hostile command 隔离明确留给可审计外部 host，而不是由 Forge 主进程伪装或由声明本身冒充实证 | 在 Forge 主进程直接运行项目测试；模型/provider 提交 independent evidence；无限等待或无 cleanup ack；不给 diff/base 输入却声称检查 diff；删除失败 verifier 成员；只验 result 不验 observation；静默收紧 seal v2；把 callback factory 当认证；明文保留 hidden oracle 或错误；任意 verifier set digest；在 B07 推导 completion |

| PD18 | Outcome evaluator 与硬门禁 | manifest v5 为每个 case 固定 outcome contract ref/digest，并绑定 objective、fixture、权威 requirement ref/digest、完整 verifier id/definition manifest 与精确 blocker outcome/reason；不读取 Skill、stage、action path、arm、model 或成本代理。B08 从 B05–B07 已验真事实派生逐项状态、Envelope/result/observation trace、claim alignment 和 success/partial/correct_block/infrastructure_error/fail；evidence-integrity、authorization/scope/safety、required hidden verifier failure、false completion 先执行稳定 hard gate，不用总分补偿，hard constraint unknown 或 runner 非完成不能变成 partial/success。report 保持不可改，内容寻址 outcome sidecar 由 seal v4 绑定；可信打开从同一 no-follow bounded file snapshot 重算并拒绝 hardlink。v1–v3 保持原语义且没有 outcome | 模型越强越可自由选择直接动作、跳过或组合能力，Forge 只约束外部目标、安全、证据与状态更新；B09 可复用一个已验真、动作路径中立的结果入口 | 用 Skill 命中/阶段完成/模型名评分；调用方替换 policy 或 verifier；自然语言自述直接判 success；平均分掩盖越权；block 自述替代 blocker oracle；把 infra 当部分进展；改写 report；验证后按路径重开；只验 sidecar digest 不从 retained evidence 重算；静默把 v3 宣称为 B08 |
| PD19 | B10 生产运行迁移 | Kernel 通过始终加载的 `AGENTS.md`/host instruction 投影，不新增 `kernel` Skill；根代理是 Chain Owner；Skill 只返回局部证据包且不自动级联；verification 始终执行，independent review 按风险升级；0.52.0 固定链只作显式兼容与实验基线 | 把不随模型能力过期的外部约束和会产生能力上限的方法脚手架分开；允许先移除生产路径约束，同时不提前声称 adaptive effectiveness | 新增 mandatory orchestrator/kernel Skill；等待 B09 才允许 direct action；把 self-check 冒充独立 review；删除 legacy baseline |

PD11 的非干扰边界已由 PD19/B10 投影到生产运行契约。B10 只改变允许的行动路径，不证明 adaptive 优于 kernel-only；任何 effectiveness 结论仍必须等待 B09、真实多 run 和外部证据。
...
- always-on Kernel、legacy compatibility chain 和全部 SKILL.md 受上下文预算 gate 约束。
...
- B08 的 policy/seal digest 提供可检测篡改和同组一致性，不提供外部 scheduler、verifier host 或 evidence store 的密码学身份认证；拥有整条存储链写权限的同一主体仍可重造自洽证据，真实结论需要不可变存储或外部签名 trust root。
- 生命周期 Skill 仍允许平台隐式发现；description 只是能力信号，不是 host 强制路由。未初始化 `AGENTS.md` 或等价 always-on policy 的项目只能获得 best-effort Skill 指导，不能声称 Kernel enforcement。
