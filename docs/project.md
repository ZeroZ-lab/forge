...
| PD17 | 外部 verifier host | B07 把 test/build/typecheck、host-private hidden assertion 与 captured-diff 检查归一成版本化 verifier runtime；verifier-set digest 绑定 host definition 和全部 adapter definition。只有 B05 注入且与受控 verifier set 相符的 runtime 能运行；B04 在 capsule 清理前执行、复查 workspace 未被 verifier 改写，并在 report 首次发布时写 independent evidence、结构化 result 与 B06 claim Envelope。结果区分 passed/task_failed/unavailable/infrastructure_error，但不判断目标 outcome | 让 verifier failure 成为不能被模型后续文本覆盖的运行事实，同时把 hostile command 隔离、timeout 和输出边界明确留给外部 host，而不是由 Forge 主进程伪装 | 在 Forge 主进程直接运行项目测试；模型/provider 提交 independent evidence；把 callback factory 当认证；明文保留 hidden oracle 或错误；任意 verifier set digest；verifier 后改 report；在 B07 推导 completion |
