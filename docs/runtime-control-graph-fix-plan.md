...
`signal_routes.to` 如果不是 24 个协议 skill，必须属于上述白名单。
...
- 为 24 个协议 skill 补 `stage_next`、`feedback_to`、`quality_gates`、`signal_routes`。
...
- `stage_next/feedback_to/quality_gates` 只引用 24 个协议 skill。
