...
**第一层：Smoke Gate（D9：Evidence over claims）**
...
**不变原则**：
- Smoke Gate 通过 + 测试通过 + 零 L1 偏差 = 任务完成（D9：Evidence over claims）
...
**出口**：src/ + tests/ 已生成 · Smoke Gate 通过（编译零错误 + 服务可启动 + 基础响应正常）· 所有测试通过 · 代码和文档对齐 · 用户确认（D9：Evidence over claims）
...
- Smoke Gate 未执行就声明完成 → 强制先跑 Smoke Gate（D9：Evidence over claims）
