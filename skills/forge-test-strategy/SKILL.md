...
**出口条件**：
- 生成了 testing/contract.md（T1-T5 决策完整）
- 测试框架已选定
- 覆盖矩阵已填写（每个端点/模块的覆盖情况）
- 测试规范已明确（命名、隔离、环境）
- 用户确认是否继续（进入代码生成）
...
- 没有选择理由 → 强制补充（"为什么选这个测试类型？被拒方案是什么？"）
- 追求 100% 覆盖率 → 强制纠正（"100% 覆盖率是虚荣指标，有效覆盖才是目标"）
- 测试间互相依赖 → 强制隔离（"测试 A 的输出不能是测试 B 的输入"）
- 测试只在本地跑 → 强制 CI（"本地跑的测试约等于没跑"）
- 测试时间超过 10 分钟 → 强制优化（"太慢的测试不会被运行"）
