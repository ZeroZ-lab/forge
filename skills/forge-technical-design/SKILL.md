...
description: 架构权衡与技术选型——每个选择都是 trade-off，不存在最优解。用户说"技术设计"、"架构"、"技术选型"、"服务划分"、运行 /forge-init、或需要做架构决策和技术 trade-off 时触发。
---
...
**下游**：project.md「技术决策」交给 /forge-detail（详设）和代码生成

**和详设阶段的切法**：
- technical-design 定义**架构层**（单体/微服务、技术栈、服务划分、部署架构）
- /forge-detail 定义**详设层**（API 合约、数据模型、组件规格）
...
**不应包含**：具体 API 设计（/forge-detail）· 具体代码实现（代码生成）· 具体测试用例（测试阶段）
...
**出口**：project.md「技术决策」已填写 · 架构模式已确定 · 技术选型已确定 · 用户确认进入 /forge-detail

## 何时不使用

纯前端项目 · 已有完整技术架构（直接进入 /forge-detail）· 技术栈已确定（简化选型流程）
...
下一步你可以：
  /forge-detail  — 进入详设阶段（API + 数据库 + 前端）
```
