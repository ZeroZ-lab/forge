---
name: api-design
description: 资源导向的 API 合约设计——D1-D7 决策点，产出能重建系统的文档树。用户说"API 设计"、"设计端点"、"API 合约"、或需要设计 REST/GraphQL API 时触发。
---
...
| 决策点 | AI 角色 | 行为 |
|--------|---------|------|
| D1 资源建模 | 资源分析师 | 从 PRD 推导核心实体，建议资源关系 |
| D2 分页 | 数据量评估者 | 根据数据量级推荐分页策略（page vs cursor） |
| D3 错误格式 | 标准推荐者 | 搜索行业标准（RFC 9457、Google API），推荐最佳实践 |
| D4 权限 | 安全评估者 | 根据安全等级推荐 403/404 策略 |
| D5 幂等 | 副作用分析者 | 从端点推导哪些需要幂等，推荐策略 |
| D6 并发 | 冲突风险者 | 从协作模式推导并发控制需求 |
| D7 认证 | 架构匹配者 | 从系统架构推导认证方式 |
...
**出口**：api/contract.md（D1-D7 完整）+ modules/*.md（接口合约完整）+ 共享数据模型
...
1. **追加 feature changelog.md**：
   ...
   ### v{版本} — {日期} — API 详设
   - **触发**：{用户说的一句话}
   - **产出**：api/contract.md（D1-D7）+ {N} 个 modules
   ```
