---
name: api-design
description: 资源导向的 API 合约设计——FD1-FD7 决策点，产出能重建系统的文档树。用户说"API 设计"、"设计端点"、"API 合约"、或需要设计 REST/GraphQL API 时触发。
---
...
| 决策点 | AI 角色 | 行为 |
|--------|---------|------|
| FD1 资源建模 | 资源分析师 | 从 PRD 推导核心实体，建议资源关系 |
| FD2 分页 | 数据量评估者 | 根据数据量级推荐分页策略（page vs cursor） |
| FD3 错误格式 | 标准推荐者 | 搜索行业标准（RFC 9457、Google API），推荐最佳实践 |
| FD4 权限 | 安全评估者 | 根据安全等级推荐 403/404 策略 |
| FD5 幂等 | 副作用分析者 | 从端点推导哪些需要幂等，推荐策略 |
| FD6 并发 | 冲突风险者 | 从协作模式推导并发控制需求 |
| FD7 认证 | 架构匹配者 | 从系统架构推导认证方式 |

## 决策点

> API 领域的决策编号使用 FD 前缀（Feature Decision）。项目级共享决策使用 PD 前缀，记录在 project.md。数据库领域使用 DB 前缀，记录在 database/contract.md。
...
**出口**：api/contract.md（FD1-FD7 完整）+ modules/*.md（接口合约完整）+ 共享数据模型
...
1. **追加 feature changelog.md**：
   ...
   ### v{版本} — {日期} — API 详设
   - **触发**：{用户说的一句话}
   - **产出**：api/contract.md（FD1-FD7）+ {N} 个 modules
   ```
