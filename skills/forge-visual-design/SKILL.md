...
**AI 行为**：从 DESIGN.md 生成 design-preview.html（单文件，零依赖，浏览器直接打开），包含：
- 色板（真实色块）· 字体层级（真实排版）· 间距系统（可视化）
- 组件展示（所有变体和状态）· 页面布局示例

发现不协调 → 改 DESIGN.md → 重新生成预览。在写代码之前，设计系统已经视觉验证过了。
